...
void show_font2(Font *ip, char letter, int offx, int offy, int color);
...
/*void putStringLarge2(Font *fp, int x, int y, char *p, int color)
{
    int i;
    for(i=0; (unsigned int) i<strlen(p); i++) {
        show_font2(fp,  p[i], x+i*fp->width, y, color);
    }
}*/
...
void show_font2(Font *ip, char letter, int offx, int offy, int color)
{
    ...
    for (y=0; y<ip->height; y++) {
            ...
            c = getpixel(ip, x, y+shift);
            if (c!=' ')drawChar('@',x+offx,y+offy,0,color);
            }
    }
}
