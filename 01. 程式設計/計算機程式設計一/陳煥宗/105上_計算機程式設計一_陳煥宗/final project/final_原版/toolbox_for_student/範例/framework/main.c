    ...
    while (!IsEnding) {
        ...
        if (cur_tick-last_tick > REFRESH_RATE(state)) {
            ...
            for(j=0;j<=min(i,word_number);j++){
                if(object[j].exist==1){
                    //putStringLarge(font_game,object[j].x,object[j].y,c,15); //改
            ...
            if(KEY_DOWN(key_val[k]) && !key_down[k]) {
                ...
                switch (key_val[k]) {
                    ...
                    case VK_RETURN:
                            ...
                            else if (IsOnItem(&suspend, 2)) {  //回到menu //??
                                pauseAudio(&audio);
                                return 3;
                            }
                        break;
...
int about(void)
{
    ...
    //char detail[10][200]={"Normal mode","try to avoid the letters bumping into the ground,","you only have three lives.",
                     //"Special mode:","you will have some special props.","TAB->fire all the letters；ctrl->one more heart"};//??
