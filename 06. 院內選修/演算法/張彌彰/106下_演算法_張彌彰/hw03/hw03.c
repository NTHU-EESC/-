/*************************************
  EE3980 Algorithms HW03 Trading Stock
...
RETval MaxSubArrayXB(STKprice *A, int begin, int mid, int end); 
													//cross boundary 
void PrintResult(STKprice *A, RETval result, double time);  //as its name
...
void PrintResult(STKprice *A, RETval result, double time){
	int low = result.low;
...
int main()
{
	...
	t = GetTime() - t;
	PrintResult(Prices,result,t/1000);
