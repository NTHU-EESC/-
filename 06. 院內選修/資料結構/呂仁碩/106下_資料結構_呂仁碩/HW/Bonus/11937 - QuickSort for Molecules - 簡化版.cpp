...
{
    for(int i=0; i<num; i++) if(mole_array[i].mass==0) mole_array[i].mass = SetMass(mole_array[i].formula);
    QuickSort(0, num-1);
}
...
double Molecule::SetMass(string formulaa)
{
    ...
    string tmp_mole = ""; string tmp_coef = "";
    double mole_mass = 0; double mass = 0;
    string tmp = "";
    int parenthesis = 0;
    for(int i=0; i<formulaa.length(); i++){
