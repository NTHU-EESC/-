...
string postfix(char*way); //infix -> postfix & print the answer
...
int main(void)
{
    ...
    for(int i=0; i<n; i++){
        ...
        for(int j=0; j<height; j++){
            ...
            for(int k=0; k<width; k++) fin >> maze[j][k];
        }

        //輸出
        if(i==0) fout << n << endl;
        fout << width << endl << height << endl;
        for(int j=0; j<height; j++){
        ...
        if(out){
            fout << "Yes" << endl;
            if((way[0]=='+'||way[0]=='-'||way[0]=='*'||way[0]=='/'||way[0]=='('||way[0]==')')&&
               (way[1]!='+'||way[1]!='-'||way[1]!='*'||way[1]!='/'||way[1]!='('||way[1]!=')')) fout << way[0] << " ";
            else fout << way[0];
            for(int j=1; way[j]!='#'; j++){
                if(way[j]=='+'||way[j]=='-'||way[j]=='*'||way[j]=='/'||way[j]=='('||way[j]==')'){
                   if(way[j+1]=='+'||way[j+1]=='-'||way[j+1]=='*'||way[j+1]=='/'||way[j+1]=='('||way[j+1]==')')fout << " " << way[j];
                   else fout << " " << way[j] << " ";
                }
                else fout << way[j];
            }
...
int find_way(char** maze, char*way)
{
    ...
    if(maze[0][0]=='+'||maze[0][0]=='-'||maze[0][0]=='*'||maze[0][0]=='/'||maze[0][0]==')') return 0;
    tmp* temp = new tmp ;
    ...
    if(temp->visit[height-1][width-1]){
    ...
    if(!last_one){
        if(cur_symbol=='1'||cur_symbol=='2'||cur_symbol=='3'||cur_symbol=='4'||cur_symbol=='5'||     //數字
           cur_symbol=='6'||cur_symbol=='7'||cur_symbol=='8'||cur_symbol=='9'){
            ...
            if(st->symbol=='+'||st->symbol=='-'||st->symbol=='*'||st->symbol=='/'||st->symbol=='(') return 1;
            else return 0;
        }
    ...
    for(int i=0; i<4; i++){
        ...
        if(dy==height-1 && dx==width-1){
            ...
            if(check(temp,maze[dy][dx],0) && !temp->visit[dy][dx]){
                //cout << maze[dy][dx] << " " << temp->str << endl;
                temp->visit[dy][dx] = true;
                temp->str += temp->symbol;
                temp->symbol = maze[dy][dx];
                temp->sym_y += dir[i][0]; temp->sym_x += dir[i][1];
                DFS(maze, temp);
                if(temp->visit[height-1][width-1]) return;
                temp->sym_y -= dir[i][0]; temp->sym_x -= dir[i][1];
